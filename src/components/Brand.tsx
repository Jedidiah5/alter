...
export function SiteNav({
  ...
  return (
    <header className="shrink-0 border-b border-[var(--line)] bg-[var(--surface)]">
      <div className="max-w-[1480px] mx-auto px-4 sm:px-5 h-14 flex items-center gap-3">
        ...
        <nav className="ml-auto flex items-center gap-1 overflow-x-auto no-scrollbar min-w-0">
          {links.map((l) => (
            <Link
              key={l.key}
              href={l.href}
              className={`flex-none whitespace-nowrap px-3 py-1.5 rounded-lg text-[13.5px] transition ${
                active === l.key
          ...
          <a
            ...
            className="flex-none whitespace-nowrap ml-1 px-3 py-1.5 rounded-lg text-[13.5px] text-[var(--muted)] hover:text-[var(--ink)]"
          >
            Docs ↗
          </a>
        </nav>
